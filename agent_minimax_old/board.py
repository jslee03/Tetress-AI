...
def apply_action(board, piece, color):
    for coord in piece.coords:
        board[coord] = color
    remove_line(board, piece)
...
def is_terminal(board):
    ...
    if len(get_possible_actions(board, coords_red)) == 0 and len(get_possible_actions(board, coords_blue)) == 0:
        return True
    else:
        False